...
class StatsRepository {
    ...
    public function sumYearlyExpenses($data = []) {
      ...
      // Omar Ahmed
    public function sumAllTaxes($data = []){
        //defaults
        $results = [
            'total' => 0,
            'paid_taxes' => 0,
        ...
        if (isset($data['period']) && (in_array($data['period'], ['this_year', 'last_year']) || is_numeric($data['period']))) {
            ...
            $invoicesQuery = $invoicesQuery->whereYear('bill_date', $year);
            $expensesQuery = $expensesQuery->whereYear('expense_date', $year);
        }

        $results['total'] = $invoicesQuery->sum('bill_tax_total_amount') ?? 0;

        $results['paid_taxes'] = $expensesQuery->whereHas('category', function ($q) {
            $q->where('category_name', 'tax');
        })->sum('expense_amount') ?? 0;

        $results['to_pay'] = $results['total'] - $results['paid_taxes'];
        return $results;
    }
}