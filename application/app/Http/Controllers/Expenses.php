...
class Expenses extends Controller {
    ...
    public function index(ProjectRepository $projectrepo, CategoryRepository $categoryrepo) {
        ...
        $page = $this->pageSettings('expenses');
           ...
           //default to current year only on the initial expense list load, not on filter/search requests
        if (!request()->filled('filter_year') && !request()->filled('expenseresource_id') && !request()->filled('expenseresource_type') && request('action') !== 'search') {
            request()->merge([
