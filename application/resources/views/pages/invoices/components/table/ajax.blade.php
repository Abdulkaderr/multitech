    ...
    <tr id="invoice_{{ $invoice->bill_invoiceid }}">
        ...
        @if (config('visibility.invoices_col_client'))
            <td class="invoices_col_company" id="invoices_col_company_{{ $invoice->bill_invoiceid }}">
                <a
                    href="/clients/{{ $invoice->bill_clientid }}">{{ str_limit($invoice->client_company_name ?? '---', 12) }}</a>
            </td>
        @endif
        @if (config('visibility.invoices_col_project'))
            <td class="invoices_col_project" id="invoices_col_project_{{ $invoice->bill_invoiceid }}">
                <a
                    href="/projects/{{ $invoice->bill_projectid }}">{{ str_limit($invoice->project_title ?? '---', 12) }}</a>
            </td>
        @endif
        ...
        <td class="invoices_col_status" id="invoices_col_status_{{ $invoice->bill_invoiceid }}">
            ...
            <span
                class="label {{ runtimeInvoiceStatusColors($invoice->bill_status, 'label') }}">{{ runtimeLang($invoice->bill_status) }}</span>
            ...
            @if (config('system.settings_estimates_show_view_status') == 'yes' &&
                    auth()->user()->is_team &&
        ...
        <td class="invoices_col_action actions_column" id="invoices_col_action_{{ $invoice->bill_invoiceid }}">
            ...
            <span class="list-table-action dropdown font-size-inherit">
                ...
                @if (config('visibility.action_buttons_edit'))
                    <a href="/invoices/{{ $invoice->bill_invoiceid }}/edit-invoice"
                    ...
                    <span class="list-table-action dropdown font-size-inherit">
                        ...
                        <div class="dropdown-menu" aria-labelledby="listTableAction">
                            @if (config('visibility.action_buttons_edit'))
                                ...
                                <!--    href="{{ url('/invoices/view/' . $invoice->bill_invoiceid . '?action=preview') }}"-->
                                <!--    target="_blank">@lang('lang.invoice_url')</a>-->
