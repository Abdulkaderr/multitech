@foreach ($projects as $project)
    <tr id="project_{{ $project->project_id }}">
        ...
        <td class="col_net_profit">
            {{ runtimeMoneyFormat(($project->sum_invoices_all ?? 0) - ($project->sum_expenses ?? 0)) }}
        </td>
                    ...
                    @if (config('visibility.action_buttons_edit'))
                        @if ($project->permission_edit_project)
                            <button type="button" title="{{ cleanLang(__('lang.edit')) }}"
                                ...
                                data-modal-title="{{ cleanLang(__('lang.edit_project')) }}"
                                data-action-url="{{ urlResource('/projects/' . $project->project_id) }}"
