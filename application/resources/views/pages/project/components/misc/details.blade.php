                ...
                <div>
                    @foreach ($project->assigned as $team)
                        <span data-toggle="tooltip" title="{{ $team->first_name }} {{ $team->last_name }}"><img
                                src="{{ getUsersAvatar($team->avatar_directory, $team->avatar_filename) }}"
                        ...
                        <div>
                            @foreach ($contacts as $contact)
                                <span data-toggle="tooltip"
                                    title="{{ $contact->first_name }} {{ $contact->last_name }}"><img
                                        src="{{ getUsersAvatar($contact->avatar_directory, $contact->avatar_filename) }}"
                                        alt="user" class="img-circle avatar-xsmall"></span>
                            @endforeach
                        </div>
                    </div>
...
<!--                </div>-->
    ...
    <div class="card-body p-t-0 p-b-0">
        ...
        @if($project->project_billing_costs_estimate > 0)
                <tr>
            <td class="p-l-0 p-t-5">{{ cleanLang(__('lang.costs_estimate')) }}</td>
            <td class="font-medium p-r-0 p-t-5">
                {{ runtimeMoneyFormat($project->project_billing_costs_estimate - $project->sum_invoices_all ) }}
                <div class="progress">
                    <div class="progress-bar bg-info {{ runtimeProjectInvoicesBars($project->project_billing_costs_estimate, $project->sum_invoices_all ) }}"
                        role="progressbar"></div>
                </div>
            </td>
        </tr>
        ...
        <div class="row">
                            ...
                            <div>
                                <div class="panel-label p-b-3">{{ cleanLang(__('lang.costs_estimate')) }}</div>
                                <div>{{ runtimeMoneyFormat($project->project_billing_costs_estimate) }}</div>
                            </div>
        ...
        @foreach ($fields as $field)
            @if ($field->customfields_show_project_page == 'yes')
                <div class="x-each-field m-b-18">
                    ...
                    <div class="x-content">{!! clean(customFieldValue($field->customfields_name, $project, $field->customfields_datatype)) !!}</div>
                </div>
            @endif
        @endforeach
