				...
				if (typeof value == 'object') {
					if (value.selector != undefined && value.action != undefined && value.value != undefined) {
						...
						if (value.action == 'replace') {
						    	//destroy sortable widgets before replacing content to prevent errors
							try {
								$(value.selector).find('.ui-sortable').sortable('destroy');
							} catch (e) {
								//ignore errors if sortable is not initialized
							}
...
//abdul start
	try {
			...
			NXAJAX.log("[ajax] reinitialise_dom() - error in NXbootstrap():", error);
		}
		// abdul end
